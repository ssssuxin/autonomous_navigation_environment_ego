...
public:
  GridMap() {
    ...
    // for(int i=0;i<5;i++)
    // {
      // pointCloudVector.push_back(cloud1);
      // pointCloudVector.push_back(cloud2);
      // pointCloudVector.push_back(cloud3);
      // pointCloudVector.push_back(cloud4);
      // pointCloudVector.push_back(cloud5);
    // }
...
private:
  ...
  void change_origin_odom_callback(const nav_msgs::msg::Odometry::SharedPtr odom);
  ...
  void cloudCallback_lidar(const sensor_msgs::msg::PointCloud2::SharedPtr img);
  void cloudCallback_copy();//这个是为了让局部区域移动的时候给局部地图实时更新的
  ...
  rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr indep_cloud_sub_lidar;
  ...
  double x_size_map, y_size_map, z_size_map;
  double x_center_of_uav_area,y_center_of_uav_area,z_center_of_uav_area;
  pcl::PointCloud<pcl::PointXYZ> latest_cloud,latest_cloud_copy;
  // std::vector<pcl::PointCloud<pcl::PointXYZ>> 
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1,cloud2,cloud3,cloud4,cloud5;
  std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> pointCloudVector;
  pcl::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> TMP_cloud;
  int ind_count=0;
  int pointCloudVector_size;
};
