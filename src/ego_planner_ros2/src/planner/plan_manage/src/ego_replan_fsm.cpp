...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(std::shared_ptr<rclcpp::Node> &nh)
  {
    ...
    way_point_showed = nh->create_publisher<geometry_msgs::msg::PointStamped>("/way_point_showed", 5);
    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      ...
      waypoint_sub_point_ = nh->create_subscription<geometry_msgs::msg::PointStamped>("/move_base_simple/goal_point",1,std::bind(&EGOReplanFSM::waypointCallback_point, this, std::placeholders::_1));
    }
  ...
  void EGOReplanFSM::waypointCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("waypointCallback"), "msg->pose.position.z < -0.1; ignore");
      // std::cout<<""<<std::endl;
      return;
    }
    ...
    // trigger_ = true;
    init_pt_ = odom_pos_;
    double A=msg->pose.position.z;
    if(msg->pose.position.z==0)
    {
      A=1;
    }
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, A);
    last_goal_wp_fromcallback = end_wp;
    odom_pos_com_with_last_goal = odom_pos_;
    planNextWaypoint(end_wp);
    geometry_msgs::msg::PointStamped pose_msg;
    pose_msg.header.frame_id="map";
    pose_msg.point.x=msg->pose.position.x;
    pose_msg.point.y=msg->pose.position.y;
    pose_msg.point.z=msg->pose.position.z;
    way_point_showed->publish(pose_msg);
  }

  void EGOReplanFSM::waypointCallback_point(const geometry_msgs::msg::PointStamped::SharedPtr msg)
  {
    if (msg->point.z < -0.1)
    {
      RCLCPP_ERROR(rclcpp::get_logger("waypointCallback"), "msg->point.z < -0.1; ignore");
      // std::cout<<""<<std::endl;
      return;
    }
    ...
    // trigger_ = true;
    init_pt_ = odom_pos_;
    double A=msg->point.z;
    if(msg->point.z==0)
    {
      A=1;
    }
    Eigen::Vector3d end_wp(msg->point.x, msg->point.y, A);
    last_goal_wp_fromcallback = end_wp;
    odom_pos_com_with_last_goal = odom_pos_;
    planNextWaypoint(end_wp);
    geometry_msgs::msg::PointStamped pose_msg;
    pose_msg.header.frame_id="map";
    pose_msg.point.x=msg->point.x;
    pose_msg.point.y=msg->point.y;
    pose_msg.point.z=msg->point.z;
    way_point_showed->publish(pose_msg);
  }
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    {
      std::cout<<"魔改EMERGENCY_STOP到WAIT_TARGET"<<std::endl;
      new_state = FSM_EXEC_STATE::WAIT_TARGET;
    }
  ...
  {
    // if(fail_times_count>20)
    // {
    //   if(fail_times_count>=200)
    //   {
    //     fail_times_count=200;
    //   }
    //   Eigen::Vector3d new_wp;
    //   new_wp.x() = (last_goal_wp_fromcallback.x()-odom_pos_com_with_last_goal.x())*(200-fail_times_count-20)/200 + odom_pos_com_with_last_goal.x();
    //   new_wp.y() = (last_goal_wp_fromcallback.y()-odom_pos_com_with_last_goal.y())*(200-fail_times_count-20)/200 + odom_pos_com_with_last_goal.y();
    //   new_wp.z() = (last_goal_wp_fromcallback.z()-odom_pos_com_with_last_goal.z())*(200-fail_times_count-20)/200 + odom_pos_com_with_last_goal.z();
    //   std::cout<<"reset the goal!   "<<fail_times_count<<std::endl;
    //   std::cout<<"("<<last_goal_wp_fromcallback.x()<<","<<last_goal_wp_fromcallback.y()<<","<<last_goal_wp_fromcallback.z()<<
    //             ");   ("<<odom_pos_com_with_last_goal.x()<<","<<odom_pos_com_with_last_goal.y()<<","<<odom_pos_com_with_last_goal.z()<<
    //             ");   ("<<new_wp.x()<<","<<new_wp.y()<<","<<new_wp.z()<<")"<<std::endl;
    //   fail_times_count=0;
    //   planNextWaypoint(new_wp);
    //   std::cout<<"reset the goal!"<<std::endl;
    //   std::cout<<"reset the goal!"<<std::endl;
    //   std::cout<<"----------------"<<std::endl;
      ...
      if (occ)
      {
        ...
        else
        {
          if (t - t_cur < emergency_time_) // 0.8s of emergency time
          {
            ...
            // changeFSMExecState(EMERGENCY_STOP, "SAFETY");
            std::cout<<"EMERGENCY BUT KEEP WORKING!!!"<<std::endl;
          }
          else
          {
